...
import { Lock, X, Package, ShoppingBag, Star, Trash2, Plus, Upload, CheckCircle, Clock, Sparkles } from 'lucide-react';
...
export default function Admin({ onClose }: { onClose: () => void }) {
  ...
  const addProduct = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!pName.trim() || !pPrice.trim() || pImgs.length === 0) { setErr('Name, price and at least one image are needed.'); return; }
    try {
      await apiPost('/api/products', {
        name: pName, price: Number(pPrice), image_url: pImgs[0],
        images: pImgs,
        description: pDesc, category: pCat || 'Boutique', stock: Number(pStock) || 0,
      });
      setPName(''); setPPrice(''); setPCat(''); setPDesc(''); setPImgs([]); setPStock('1');
      setErr('');
      loadAll();
    } catch (e: any) { setErr(e.message); }
  };

  const delProduct = async (id: number) => { await apiDelete('/api/products', { id }); loadAll(); };
  const setOrderStatus = async (id: number, status: string) => { await apiPut('/api/orders', { id, status }); loadAll(); };
  const delOrder = async (id: number) => { await apiDelete('/api/orders', { id }); loadAll(); };
  const delReview = async (id: number) => { await apiDelete('/api/reviews', { id }); loadAll(); };
    ...
    <div className="fixed inset-0 z-[70] bg-[var(--cream)] overflow-y-auto">
      <div className="sticky top-0 z-10 bg-gradient-to-r from-[var(--lavender-deep)] to-[var(--pink-deep)] text-white px-4 sm:px-5 py-4 flex items-center justify-between shadow-luxe">
        <div className="flex items-center gap-2 min-w-0">
          ...
          {authed && <span className="text-[10px] bg-white/20 px-2 py-0.5 rounded-full shrink-0">live</span>}
        </div>
        <button onClick={onClose} className="p-2 rounded-full hover:bg-white/20 shrink-0"><X size={20} /></button>
      </div>
        ...
        {!authed ? (
          <motion.div key="login" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center justify-center min-h-[80vh] px-4">
            <form onSubmit={login} className="bg-white rounded-3xl shadow-luxe p-6 sm:p-8 w-full max-w-sm text-center">
              ...
              <button type="submit" className="w-full mt-4 py-3 rounded-full bg-gradient-to-r from-[var(--lavender-deep)] to-[var(--pink-deep)] text-white font-medium">Enter</button>
            </form>
          </motion.div>
        ) : (
          <motion.div key="panel" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-5xl mx-auto px-3 sm:px-5 py-4 sm:py-6">
            <div className="grid grid-cols-3 gap-1.5 mb-5 bg-white rounded-2xl p-1.5 shadow-soft">
              {tabs.map(([t, label, Icon]) => (
                <button key={t} onClick={() => setTab(t)}
                  className={`inline-flex items-center justify-center gap-1.5 px-2 py-2 rounded-xl text-xs font-medium transition-all ${
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-3 gap-2">
                  {(['new', 'pending', 'complete']).map((s) => (
                    <div key={s} className="bg-white rounded-2xl p-2.5 text-center shadow-soft">
                      <div className={`text-2xl font-display font-semibold ${s === 'new' ? 'text-[var(--pink-deep)]' : s === 'pending' ? 'text-amber-500' : 'text-green-600'}`}>
                        {orders.filter((o) => o.status === s).length}
                      </div>
                      <div className="text-[10px] text-[var(--plum-soft)] capitalize flex items-center justify-center gap-1">
                ...
                {orders.length === 0 ? (
                  <p className="text-center text-[var(--plum-soft)] py-12">No orders yet 🌸</p>
                ) : orders.map((o) => (
                  <motion.div key={o.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl p-3.5 shadow-soft">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div className="min-w-0 flex-1">
                        <div className="font-semibold text-[var(--plum)] text-sm truncate">{o.customer_name}</div>
                    ...
                    <div className="text-xs text-[var(--plum-soft)] mb-2">📍 {o.address}</div>
                    <div className="text-xs text-[var(--plum-soft)] bg-[var(--lavender-soft)]/50 rounded-xl p-2 mb-3">
                      {o.items?.map((it: any) => `${it.name} ×${it.qty}`).join(', ')}
                    </div>
                    <div className="mb-2"><span className="font-semibold text-[var(--plum)] text-sm">Rs. {o.total}</span></div>
                    <div className="grid grid-cols-3 gap-1.5">
                      <button onClick={() => setOrderStatus(o.id, 'new')} className={`text-[11px] px-2 py-2 rounded-full font-medium ${o.status === 'new' ? 'bg-[var(--pink-deep)] text-white' : 'bg-[var(--pink-soft)] text-[var(--pink-deep)]'}`}>New</button>
                      <button onClick={() => setOrderStatus(o.id, 'pending')} className={`text-[11px] px-2 py-2 rounded-full font-medium ${o.status === 'pending' ? 'bg-amber-500 text-white' : 'bg-amber-100 text-amber-700'}`}>Pending</button>
                      <button onClick={() => setOrderStatus(o.id, 'complete')} className={`text-[11px] px-2 py-2 rounded-full font-medium ${o.status === 'complete' ? 'bg-green-600 text-white' : 'bg-green-100 text-green-700'}`}>Complete</button>
                      <button onClick={() => delOrder(o.id)} className="col-span-3 text-[11px] px-2 py-2 rounded-full font-medium text-red-500 bg-red-50 flex items-center justify-center gap-1">
            ...
            {tab === 'products' && (
              <div className="grid lg:grid-cols-2 gap-4">
                <form onSubmit={addProduct} className="bg-white rounded-2xl p-4 shadow-soft h-fit order-2 lg:order-1">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <input value={pPrice} onChange={(e) => setPPrice(e.target.value)} placeholder="Price (Rs.)" type="number"
                      ...
                      <input value={pStock} onChange={(e) => setPStock(e.target.value)} placeholder="Stock" type="number"
                        className="px-3 py-2.5 rounded-xl bg-[var(--lavender-soft)]/40 border border-[var(--lavender-soft)] focus:outline-none focus:border-[var(--lavender)] text-sm" />
                    </div>
                    ...
                    <div className="border-2 border-dashed border-[var(--lavender)] rounded-2xl py-4 px-3">
                      <p className="text-xs text-[var(--plum-soft)] text-center mb-3">Upload up to 4 product photos</p>
                      <div className="flex flex-wrap gap-2 justify-center">
                        {pImgs.map((url, i) => (
                          <div key={i} className="relative">
                            <img src={url} className="h-16 w-16 object-cover rounded-xl" />
                            <button type="button" onClick={() => setPImgs(prev => prev.filter((_, j) => j !== i))}
                              className="absolute -top-1 -right-1 bg-red-400 text-white rounded-full w-5 h-5 text-xs flex items-center justify-center font-bold">×</button>
                          </div>
                        ))}
                        {pImgs.length < 4 && (
                          <label className="h-16 w-16 flex flex-col items-center justify-center gap-1 border-2 border-dashed border-[var(--lavender)] rounded-xl cursor-pointer hover:bg-[var(--lavender-soft)]/30">
                            <Upload size={18} className="text-[var(--lavender-deep)]" />
                            <span className="text-[10px] text-[var(--plum-soft)]">{uploading ? '...' : '+ Add'}</span>
            ...
            {tab === 'reviews' && (
              <div className="space-y-3">
                {reviews.length === 0 ? (
                  ...
                  <div key={r.id} className="bg-white rounded-2xl p-3.5 shadow-soft flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
